...
zcat /tmp/wiki-import/page.sql.gz | docker compose exec -T db mysql -u"$MARIADB_USER" -p"$MARIADB_PASSWORD" "$MARIADB_DATABASE"
zcat /tmp/wiki-import/page_props.sql.gz | docker compose exec -T db mysql -u"$MARIADB_USER" -p"$MARIADB_PASSWORD" "$MARIADB_DATABASE"
